...
class CartService {
  ...
  static async getVNPayPayment(token, { totalAmount, bankCode }) {
    try {
      const response = await axios.get(`${CartService.BASE_URL}/money/vn-pay`, {
  ...
  static async applyCoupon(token, couponCode, totalAmount) {
    try {
      const response = await axios.get(
        `${CartService.BASE_URL}/products/calculate?couponCode=${couponCode}&totalAmount=${totalAmount}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
