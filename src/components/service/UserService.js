...
class UserService {
    ...
    static async getProflie(token) {
        try {
            const response = await axios.get(`${UserService.BASE_URL}/api/customer/profile`, {
    ...
    static async changeProflie(token, userRequest) {
        try {
            const response = await axios.post(`${UserService.BASE_URL}/api/customer/change-profile`, userRequest, {
                headers: { Authorization: `Bearer ${token}` }
            })
            return response.data;
        } catch (err) {
    ...
    static async updateUser(token, userUpdate) {
        try {
            const response = await axios.post(`${UserService.BASE_URL}/api/admin/update-user`, userUpdate, {
                headers: { Authorization: `Bearer ${token}` }
            })
            return response.data;
        } catch (err) {
    ...
    static async register(email, username, password) {
        try {
            const response = await axios.post(`${UserService.BASE_URL}/api/auth/register`, { email, username, password })
            return response.data;
        } catch (err) {
            throw err;
        }
    ...
    static async getALlUserForAdmin(token, key, noPage, categoryName) {
        try {
            const response = await axios.get(`${UserService.BASE_URL}/api/admin/get-all-user?key=${key}&&noPage=${noPage}&&categoryName=${categoryName}`, {
                headers: { Authorization: `Bearer ${token}` }
            })
            return response.data;
        } catch (err) {
