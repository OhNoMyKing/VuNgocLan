        ...
        <div classNameName="" style={{ marginTop: '20px' }}>
            <div className="container">
                <article className="card">
                    <header className="card-header"> <button className={`btn ${status === 0 ? 'text-danger' : ''}`} >Quản lý đơn hàng</button> / <button className={`btn ${status === 1 ? 'text-danger' : ''}`} onClick={statusOne}>Đã tiếp nhận</button> / <button className={`btn ${status === 2 ? 'text-danger' : ''}`} onClick={statusTwo}>Đã đóng gói</button> / <button className={`btn ${status === 3 ? 'text-danger' : ''}`} onClick={statusThree}>Đang giao</button> / <button className={`btn ${status === 4 ? 'text-danger' : ''}`} onClick={statusFour}>Đã giao</button> </header>
                            ...
                            <div className="">
                                ...
                                <article className="card">
                                    <div className="card-body row">
                                        <div className="col"> <strong>Thời gian tạo:</strong> <br />{item.createDate} </div>
                                        {/* <div className="col"> <strong>Địa chị nhận:</strong> <br />  {item.shippingAddress} </div> */}
                                        <div className="col"> <strong>Người nhận:</strong> <br /> {item.receiver}, | <i className="fa fa-phone"></i>{item.phone} </div>
                                        <div className="col"> <strong>Shipment:</strong> <br /> {item.shippingResponse.shippingMethod} (${item.shippingResponse.shippingFee}) </div>
                                        <div className="col"> <strong>Tổng giá trị sản phẩm:</strong> <br /> ${item.totalOrder} </div>
                                        <div className="col"> <strong>Giá trị đơn hàng:</strong> <br /> ${item.totalOrder + item.shippingResponse.shippingFee} </div>

                                    </div>
                                </article>
                                <div className="track">
                                    <div className={`step ${item.status >= 1 ? 'active' : ''}`} onClick={() => setStatusOrderOneByIndex(index)}> <span className="icon"> <i className="fa fa-check"></i> </span> <span className="text">Đã tiếp nhận</span> </div>
                                    <div className={`step ${item.status >= 2 ? 'active' : ''}`} onClick={() => setStatusOrderTwoByIndex(index)} > <span className="icon"> <i className="fa fa-user"></i> </span> <span className="text"> Đã đóng gói</span> </div>
                                    <div className={`step ${item.status >= 3 ? 'active' : ''}`} onClick={() => setStatusOrderThreeByIndex(index)} > <span className="icon"> <i className="fa fa-truck"></i> </span> <span className="text"> Đang giao </span> </div>
                                    <div className={`step ${item.status >= 4 ? 'active' : ''}`} onClick={() => setStatusOrderFourByIndex(index)} > <span className="icon"> <i className="fa fa-box"></i> </span> <span className="text">Đã giao</span> </div>
                                </div>
                                <hr />
                                <ul className="row">
